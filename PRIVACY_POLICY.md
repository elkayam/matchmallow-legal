...
14. [Push Notifications & Opt-Out](#14-push-notifications--opt-out)
15. [Cookies & Tracking](#15-cookies--tracking)
...
- City and country (used to estimate distance — see [Section 6](#6-location-city-level-only-no-gps))
- Profile photos
- Short bio
- Interests
...
- **Derived/computed** by us — for example, we estimate the approximate distance between members using the public coordinates of the cities they select (see [Section 6](#6-location-city-level-only-no-gps)).
...
- **Send push notifications** — such as new match and new message alerts (you can opt out — see [Section 14](#14-push-notifications--opt-out));
...
|---|---|---|
| **Microsoft Azure** | Hosting and database | All app data stored server-side |
| **Cloudflare R2** | Photo storage | Profile photos |
